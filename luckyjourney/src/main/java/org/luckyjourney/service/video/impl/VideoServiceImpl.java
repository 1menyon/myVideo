...
public class VideoServiceImpl extends ServiceImpl<VideoMapper, Video> implements VideoService {
    ...
    public void publishVideo(Video video) {
        ...
        if (videoId != null) {
            ...
            if (!(video.buildVideoUrl()).equals(oldVideo.buildVideoUrl()) || !(video.buildCoverUrl()).equals(oldVideo.buildCoverUrl()) ||
                    !(video.buildLabel()).equals(oldVideo.buildLabel()) || !(video.getTypeId()).equals(oldVideo.getTypeId())) {
        ...
        // TODO 这里默认是慢审核，应该修改为跟用户有关，会员就是快审核
    ...
    public void historyVideo(Long videoId, Long userId) {
        ...
        if (o == null) {
            ...
            // 用户的历史观看记录 存入redis中 设置过期时间7天
            // （key："user:history:video:观看用户id"     sorce:观看时间    value:视频对象）
    ...
    public Collection<Video> followFeed(Long userId, Long lastTime) {
        ...
        if (ObjectUtils.isEmpty(set) || set == null) {
    ...
    /**
     *   将当前用户关注的人的发件箱中的视频 拉取到当前用户的收件箱
     * */
    @Override
    public void initFollowFeed(Long userId) {
        // 获取所有关注的人（某些大v的粉丝可能有几千万，但是某个人的关注列表绝对没这么夸张，关注数量相对很少，可以用mysql存）
    ...
    public void setUserVoAndUrl(Collection<Video> videos) {
        if (!ObjectUtils.isEmpty(videos)) {
            ...
            for (Video video : videos) {
                ...
                // video表的url字段对应file表的视频id
                fileIds.add(video.getUrl());
                // video表的cover字段对应file表的封面图片id
                fileIds.add(video.getCover());
            }
            // 以下返回map集合，键为文件id/用户id，值为文件/用户，java8流式编程
