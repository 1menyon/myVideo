...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public boolean register(RegisterVO registerVO) throws Exception {
        ...
        // 这里需要先保存用户到数据库，数据库主键自增，产生id，后续再将用户id存入收藏夹中userid字段
    ...
    public Page<User> getFollows(Long userId, BasePage basePage) {
        ...
        // key：userId   value:是否互相关注
        Map<Long,Boolean> map = new HashMap<>();
        for (Long followId : followIds) {
            // fans.contains(followId)：是否互相关注了
    ...
    public void subscribe(Set<Long> typeIds) {
        ...
        // 将用户订阅的  所有兴趣分类下的 所有标签都放入 用户兴趣模型modelVO中
    ...
    public void updateUser(UpdateUserVO user) {
        // 这里应该修改，应该在UpdateUserVO中设置userId属性，并在controller中注入userId值（才能在微服务架构下也正常使用）
        ...
        if(user.getDefaultFavoritesId()!=null){
            // 校验默认收藏夹 是否存在
